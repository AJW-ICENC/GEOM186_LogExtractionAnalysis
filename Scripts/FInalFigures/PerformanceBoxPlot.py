...
Peak Memory Usage Box Plots by Development and Deployment Period
...
OUTPUT_FIGURE = str(
    Path(OUTPUT_DIR) / "PeakMemory_Boxplots_ByPeriod_V2.png"
)

OUTPUT_STATS_CSV = str(
    Path(OUTPUT_DIR) / "PeakMemory_Boxplots_ByPeriod_Stats_V2.csv"
)
...
def calculate_period_stats(values):
    ...
    if values.empty:
        ...
        return {
            ...
            "p90_mb": None,
            "p95_mb": None,
            "lower_outlier_threshold_mb": None,
            "upper_outlier_threshold_mb": None,
            "lower_outlier_count": 0,
            "upper_outlier_count": 0,
            "total_outlier_count": 0,
            "outlier_percentage": None,
            "non_outlier_min_mb": None,
            "non_outlier_max_mb": None,
        }
    ...
    return {
        ...
        "iqr_mb": iqr,
        "max_mb": values.max(),
        "range_mb": values.max() - values.min(),
        "p05_mb": values.quantile(0.05),
        "p10_mb": values.quantile(0.10),
        "p90_mb": values.quantile(0.90),
        "p95_mb": values.quantile(0.95),
        "lower_outlier_threshold_mb": lower_threshold,
        "upper_outlier_threshold_mb": upper_threshold,
        ...
        "non_outlier_min_mb": non_outliers.min() if not non_outliers.empty else None,
        "non_outlier_max_mb": non_outliers.max() if not non_outliers.empty else None,
    }
...
for service in SERVICES:
    ...
    baseline_mean = baseline["mean_mb"].iloc[0]
    baseline_median = baseline["median_mb"].iloc[0]
    baseline_iqr = baseline["iqr_mb"].iloc[0]
    baseline_cv = baseline["coefficient_of_variation"].iloc[0]

    stats_df.loc[
        service_mask,
        "mean_change_from_after_sprint_1_mb",
    ] = (
        stats_df.loc[
            service_mask,
            "mean_mb",
    ...
    stats_df.loc[
        service_mask,
        "median_change_from_after_sprint_1_mb",
    ] = (
        stats_df.loc[
            service_mask,
            "median_mb",
    ...
    stats_df.loc[
        service_mask,
        "iqr_change_from_after_sprint_1_mb",
    ] = (
        stats_df.loc[
            service_mask,
            "iqr_mb",
...
for ax, service in zip(
    ...
    boxplot = ax.boxplot(
        ...
        flierprops=dict(
            marker="o",
            markerfacecolor=OUTLIER_COLOUR,
            markeredgecolor=OUTLIER_COLOUR,
            markersize=2.4,
            alpha=0.3,
        ),
    )
    ...
    service_min = service_df["peak_memory_mb"].min()
    service_max = service_df["peak_memory_mb"].max()
    service_range = service_max - service_min
...
print("Peak memory box plot analysis complete.")